...
**Обязательные права доступа**: `crm`, `user_brief`, `pull`, `placement`, `userfieldconfig`, `bizproc`
...
- **Основной URL приложения / Путь вашего обработчика**: `[технический-домен]/`
- **Путь для первоначальной установки**: `[технический-домен]/install`
- **Разрешения (скоупы)**: `crm`, `user_brief`, `pull`, `placement`, `userfieldconfig`, `bizproc`

Установка выполняется через frontend install wizard: Bitrix24 открывает страницу `/install`, backend endpoint `/api/install` сохраняет установку и auth-данные как один из шагов wizard, затем install-страница выполняет регистрацию роботов и завершает установку через `installFinish()`. После завершения установки Bitrix24 открывает основной URL приложения `/`. Если вы добавили новые install-шаги или роботов, приложение нужно переустановить в портале, чтобы wizard прошёл заново.
