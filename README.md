...
- **URL установки приложения**: `[технический-домен]/api/install`
- **Разрешения (скоупы)**: `crm`, `user_brief`, `pull`, `placement`, `userfieldconfig`, `bizproc`

Установка выполняется через frontend install wizard: backend endpoint `/api/install` сохраняет установку и auth-данные, затем install-страница последовательно выполняет дополнительные шаги и завершает установку через `installFinish()`. Если вы добавили новые install-шаги или роботов, приложение нужно переустановить в портале, чтобы wizard прошёл заново.
